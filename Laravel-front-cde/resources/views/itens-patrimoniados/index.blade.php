...
@section('title', 'Itens Patrimoniados - Controle de Estoque')
@section('page-title', 'Itens Patrimoniados')
    ...
    <div class="p-4 border-b border-slate-200">
        <div class="flex flex-wrap items-center gap-3">
            <div class="flex-1 min-w-[200px]">
                <input type="text" id="searchInput" placeholder="Buscar por patrimônio ou observação..." class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
            </div>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-secondary-light">
                    ...
                    <th class="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-secondary">Patrimônio</th>
...
<div id="createModal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50" onclick="if(event.target===this)closeModal('createModal')">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h3 class="text-lg font-semibold text-secondary">Novo Item Patrimoniado</h3>
        ...
        <form id="createForm" onsubmit="createItem(event)" class="p-6 space-y-4">
            <div>
                <label class="block text-sm font-medium text-secondary mb-1">Nº Patrimônio</label>
                <input type="number" name="patrimonio" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
            </div>
            <div>
...
<div id="editModal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50" onclick="if(event.target===this)closeModal('editModal')">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h3 class="text-lg font-semibold text-secondary">Editar Item Patrimoniado</h3>
...
async function loadItens() {
    ...
    const endpoint = showTrashed ? '/itens-patrimoniados/trashed' : '/itens-patrimoniados';
    try {
        ...
        let filtered = itens.filter(item => {
            if (searchTerm) {
                const patrimonio = String(item.patrimonio || '');
                const obs = item.observacao || '';
                if (!patrimonio.includes(searchTerm) && !obs.toLowerCase().includes(searchTerm)) return false;
            }
        ...
        tbody.innerHTML = filtered.map(item => `
            <tr class="hover:bg-secondary-light/50 transition-colors">
                ...
                <td class="px-4 py-3 text-sm font-medium text-secondary">${item.patrimonio}</td>
...
async function createItem(event) {
    ...
    try {
        await api.post('/itens-patrimoniados', data);
...
async function editItem(id) {
    try {
        await loadDependencies();
        const item = await api.get(`/itens-patrimoniados/${id}`);
...
async function updateItem(event) {
    ...
    try {
        await api.put(`/itens-patrimoniados/${data.id}`, data);
...
async function deleteItem(id) {
    ...
    try {
        await api.delete(`/itens-patrimoniados/${id}`);
...
async function restoreItem(id) {
    try {
        await api.put(`/itens-patrimoniados/${id}/restore`);
