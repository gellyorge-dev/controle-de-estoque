...
@section('title', 'Tipos - Controle de Estoque')
@section('page-title', 'Tipos')
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
    <div class="p-4 border-b border-slate-200">
        <div class="flex flex-wrap items-center gap-3">
            <div class="flex-1 min-w-[200px]">
                <input type="text" id="searchInput" placeholder="Buscar tipo..." class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
            </div>
            <label class="flex items-center gap-2 text-sm text-secondary cursor-pointer">
                <input type="checkbox" id="showTrashed" onchange="loadTipos()" class="rounded border-slate-300 text-primary focus:ring-primary/20">
                Mostrar deletados
            </label>
        </div>
    </div>
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody id="tiposTableBody" class="divide-y divide-slate-100">
            </tbody>
        </table>
    </div>
...
<div id="createModal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50" onclick="if(event.target===this)closeModal('createModal')">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h3 class="text-lg font-semibold text-secondary">Novo Tipo</h3>
        ...
        <form id="createForm" onsubmit="createTipo(event)" class="p-6 space-y-4">
            <div>
                <label class="block text-sm font-medium text-secondary mb-1">Nome do Tipo</label>
                <input type="text" name="nome_tipo" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
            </div>
...
<div id="editModal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50" onclick="if(event.target===this)closeModal('editModal')">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md mx-4" onclick="event.stopPropagation()">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            <h3 class="text-lg font-semibold text-secondary">Editar Tipo</h3>
        ...
        <form id="editForm" onsubmit="updateTipo(event)" class="p-6 space-y-4">
...
async function loadTipos() {
    ...
    const endpoint = showTrashed ? '/tipos/trashed' : '/tipos';
    try {
        const tipos = await api.get(endpoint);
        ...
        let filtered = tipos;
        if (searchTerm) {
            filtered = tipos.filter(t => t.nome_tipo?.toLowerCase().includes(searchTerm));
        }
        ...
        tbody.innerHTML = filtered.map(t => `
            <tr class="hover:bg-secondary-light/50 transition-colors">
                <td class="px-4 py-3 text-sm text-secondary">${t.id}</td>
                <td class="px-4 py-3 text-sm font-medium text-secondary">${escapeHtml(t.nome_tipo)}</td>
                <td class="px-4 py-3">
                    ${t.deleted_at
                ...
                <td class="px-4 py-3 text-right">
                    ${t.deleted_at
                        ? `<button onclick="restoreTipo(${t.id})" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-emerald-700 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition-colors">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                            Restaurar
                        </button>`
                        : `<button onclick="editTipo(${t.id})" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-primary bg-primary-light rounded-lg hover:bg-primary/10 transition-colors mr-1">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            Editar
                        </button>
                        <button onclick="deleteTipo(${t.id})" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">
        ...
        showToast('Erro ao carregar tipos: ' + (e.message || 'Erro desconhecido'), 'error');
    }
}

async function createTipo(event) {
...
async function editTipo(id) {
    try {
        const tipo = await api.get(`/tipos/${id}`);
        setFormData('editForm', { id: tipo.id, nome_tipo: tipo.nome_tipo });
        openModal('editModal');
    } catch (e) {
...
async function updateTipo(event) {
    event.preventDefault();
    const data = getFormData('editForm');
    try {
        await api.put(`/tipos/${data.id}`, data);
...
async function deleteTipo(id) {
    ...
    try {
        ...
        showToast('Tipo deletado com sucesso!');
        loadTipos();
    } catch (e) {
...
async function restoreTipo(id) {
    try {
        await api.put(`/tipos/${id}/restore`);
        showToast('Tipo restaurado com sucesso!');
        loadTipos();
    } catch (e) {
...
document.getElementById('searchInput')?.addEventListener('input', loadTipos);
